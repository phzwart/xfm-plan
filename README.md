...
A route's output has three tiers of what a person must supply. Blocked: a false assumption (with its failure mode and the alternative step) or a physical parameter with no default that has not been supplied — the route cannot run. Human input required: a decision whose `requires_human` is set (with its question and rule, or "no stated rule"), and any non-physical parameter with no default (with its prior use, when the graph records one) — the route can run, but someone has to choose. Unverified: assumptions neither confirmed nor denied, with their stated confirmation route if any — the route is valid only if these hold.
